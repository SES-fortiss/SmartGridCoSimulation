...
public class SEffINListener extends KeyAdapter implements FocusListener {
	...
	public void focusLost(FocusEvent e) {
		if (!valid) {
			String currentVal = Double.toString(o.getEffIN());
		...
		if (check) {
			...
			if (!containsNumber) {
				...
				if (num > 1) {
					...
					valid = true;
					o.setEffIN(Double.parseDouble(input));
				}
			}
		}
	}
