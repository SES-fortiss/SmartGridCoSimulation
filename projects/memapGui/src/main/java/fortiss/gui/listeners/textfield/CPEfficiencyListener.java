...
public class CPEfficiencyListener extends KeyAdapter implements FocusListener {
	...
	public void focusLost(FocusEvent e) {
		if (!valid) {
			String currentVal = Double.toString(o.getEfficiency());
		...
		if (check) {
			...
			if (!containsNumber) {
				...
				if (num > 1) {
					...
					valid = true;
					o.setEfficiency(Double.parseDouble(input));
				}
			}
		}
	}
