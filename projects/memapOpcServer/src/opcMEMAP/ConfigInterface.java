...
public class ConfigInterface {
  ...
  /**
   * Creates an UDP socket and returns the corresponding IP address. Retrieving the IP address of a
   * server is a cumbersome and this might not work on all systems. Please refer to this discussion:
  ...
  public String getHost() {
    try (final DatagramSocket socket = new DatagramSocket()) {
      socket.connect(InetAddress.getByName("8.8.8.8"), 10002); // 8.8.8.8. can be unreachable, which
                                                               // is ok
