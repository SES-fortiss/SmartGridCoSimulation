...
/**
 *
 * Topology for example with 2 buildings. 
 * Simulation of 2 Houses, 2 Days Horizon with 15 min steps - MPC deactivated (only one run)
 * Same Topology as for memap/examples/linprog and memap/examples/linprogMPC 
 * 
 * Goal is to verify result consistency in different implementations
 * 
 * @author Jan Mayer
 *
 */
@SuppressWarnings("unused")
public class Topology2Houses2Single {
	private static String simulationName = "2_Houses";
	
	// ================= Model Predictive Control (MPC) =================
	public static int N_STEPS_MPC = 12;
	...
	public static int NR_OF_ITERATIONS =  1;  //2*TIMESTEPS_PER_DAY; // X Days
	public static int N_STEPS = 12; // NR_OF_ITERATIONS + N_STEPS_MPC; // benötigt man damit der Optimierer funktioniert.
...
//  ===============================================================		
	...
	public static ActorTopology createTopology(int mpcInput,boolean memapActive){
		...
		building2.addActorAsChild(building2Name + "/SolarThermic", ActorFactory.createSolarThermic(AREA_SOLARTHERMIC * EFFICIENCY_SOLARTHERMIC, port++));		
		...
		building2.addActorAsChild(building2Name + "/CHP", ActorFactory.createCHP( QDOT_MAX_CHP, EFFICIENCY_CHP_EL, EFFICIENCY_CHP_H, port++));
