...
public class GuiController {
	...
	class BuildingControllerDeserializer implements JsonDeserializer<BuildingController> {
		...
		public BuildingController deserialize(JsonElement jsonElement, Type typeOfT, JsonDeserializationContext context)
			...
			// Attaching devices
			JsonArray demands = (JsonArray) jObject.get("demand_list");
			...
			gsonBuilder.registerTypeAdapter(CSVConsumer.class, new CSVDemandDeserializer());
			Gson gson = gsonBuilder.create();
			...
			for (JsonElement demandEl : demands) {
				JsonObject demandConfig = (JsonObject) demandEl;
				CSVConsumer demand = gson.fromJson(demandConfig, CSVConsumer.class);
				building.attach(demand);
			}
			return building;
		}
	}
	...
	class CSVDemandDeserializer implements JsonDeserializer<CSVConsumer> {
		@Override
		public CSVConsumer deserialize(JsonElement jsonElement, Type typeOfT, JsonDeserializationContext context)
			...
			String networkS = jObject.get("networkType").getAsString();
			NetworkType networkType = NetworkType.ELECTRICITY;			
			if (networkS.equals("Heat")) {
				networkType = NetworkType.HEAT;
			}
			
			String consumptionFile = jObject.get("consumptionProfile").getAsString();
			
			return new CSVConsumer(consumptionFile, networkType, 0);
		}
	}
