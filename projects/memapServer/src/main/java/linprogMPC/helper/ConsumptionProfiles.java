...
public class ConsumptionProfiles {
	...
	private HashMap<Integer, ArrayList<Double>> heatProfile = new HashMap<Integer, ArrayList<Double>>();
	...
	public ConsumptionProfiles(int nrOfProfiles) {
		...
		/*
		 * Units: kWh / minute - for 24 hours, 1440 minutes = entries
		 */
		heatProfile = readConsumption("WaermeVerbraeuche.csv", nrOfProfiles);
		electricityProfile = readConsumption("StromVerbraeuche.csv", nrOfProfiles);

		System.out.println("heatProfile: " + heatProfile.get(0).size());
		System.out.println("electricityProfile: " + electricityProfile.get(0).size());
	}

	/**
	 * Returns the current heat consumption at a given time step.
	 * 
	 * Inputs, in kW per minute, are converted to kWh and the multiplied by time
	 * step in hours
	 * 
	...
	public double getHeatConsumption(int consumptionIndex, int timestep) throws Exception {
		...
		return heatProfile.get(consumptionIndex).get(timestep) / 60;
	}
	 ...
	 */
	private void read(BufferedReader br, HashMap<Integer, ArrayList<Double>> dailyProfiles,
			HashMap<Integer, ArrayList<Double>> profiles) throws IOException, ParseException {
