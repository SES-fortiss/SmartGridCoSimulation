...
public class OpcuaClient {
	...
	LoggerFactory.getLogger(getClass()).warn("security temp dir: {}", securityTempDir.getAbsolutePath());
...
//        KeyStoreLoader loader = new KeyStoreLoader().load(securityTempDir, clientName, clientURI);
	...
	try {
	    ...
	    try {
		endpoints = UaTcpStackClient.getEndpoints(memapClient.getEndpointUrl()).get();
	    } catch (Throwable ex) {
...
//			    .setKeyPair(loader.getClientKeyPair())
		    .setEndpoint(endpoint).setIdentityProvider(memapClient.getIdentityProvider())
