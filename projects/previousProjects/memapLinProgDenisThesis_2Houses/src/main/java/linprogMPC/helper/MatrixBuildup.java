...
//		System.out.println("****************************************************************");		
	...
	private void addProducerToProblem(
		...
		for(int i = 0; i < nStepsMPC; i++) {
			...
			lb[i] = 0;
			ub[i] = producerMessage.installedPower / producerMessage.efficiency;		
		...
		if (producerMessage instanceof VolatileProducerMessage) {
			for(int i = 0; i < nStepsMPC; i++) {
				ub[i] = ((VolatileProducerMessage) producerMessage).forecast[i] / ((VolatileProducerMessage) producerMessage).efficiency;			
			}
			
//			System.out.println("Installed Pwr Producer: " + ((VolatileProducerMessage) producerMessage).installedPower + ", Efficiency Producer: " + ((VolatileProducerMessage) producerMessage).efficiency);
//			System.out.println("Limit Producer: " + ((VolatileProducerMessage) producerMessage).installedPower / ((VolatileProducerMessage) producerMessage).efficiency + " kW per timestep.");	
//			System.out.println("Max forecast = " + ((VolatileProducerMessage) producerMessage).forecast[nStepsMPC/4]);
		}
	...
	private void addCouplerToProblem(
		...
		System.out.println("Installed Pwr Coupler: " + couplerMessage.installedPower + ", Efficiency Coupler: " + couplerMessage.efficiencyHeat);
		System.out.println("Limit Coupler: " + couplerMessage.installedPower / couplerMessage.efficiencyHeat+ " kW per timestep.");
	...
	private void addStorageToProblem(
		...
		// Matrizen befüllen. Zuerst die regulären, dann die Kapazität
		...
		// Matrizen in das Gesamtsystem übernehmen
	...
	private void addMarkets(
		...
		if (producersHandled+storagesHandled+couplersHandled == problem.getNumberofProducers()+problem.getNumberofStorages()+ problem.getNumberofCouplers()) {				
			...
			for(int j = 0; j < nStepsMPC; j++) {				
				...
				problem.lambda[n_index+nStepsMPC+j] = -.1111; // -energyPrices.getElectricityPriceInEuro(cts+j)*0.5;   // electricity sell price
