	...
	public void makeDecision() {
		...
		if (ThesisTopologySimple.MEMAP_ON) {
			...
			if (GlobalTime.getCurrentTimeStep() == (ThesisTopologySimple.NR_OF_ITERATIONS-1)) {
				...
				if(ThesisSimulation.flagPlanning) {
				// Only one timestep with long MPC Horizon
					totalCostsMEMAP = solHandler.exportCostsFull(sol, problem.lambda, "CostVectorMEMAP.csv");
				}
				...
				SolutionHandler.exportVector(sol, "XvectorMEMAP.csv");
				SolutionHandler.exportVector(problem.b_eq, "ConsumptionMEMAP.csv");
				solHandler.exportMatrix(problem.a_eq, "AMatrix.csv");
				System.out.println(" &&&&&& Results saved! &&&&&");
				double costsMEMAP = solHandler.exportCostsFull(sol, problem.lambda, "CostVectorMEMAP.csv");
