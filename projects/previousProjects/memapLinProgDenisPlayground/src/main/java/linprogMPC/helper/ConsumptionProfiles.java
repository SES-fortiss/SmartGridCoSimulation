...
//	private ArrayList<Double> electricityProfile = new ArrayList<Double>();
	...
	private void read(BufferedReader br, HashMap<Integer, ArrayList<Double>> dailyProfiles, HashMap<Integer, ArrayList<Double>> profiles) throws IOException, ParseException{
		...
		while ( (zeile = br.readLine()) != null) {		
			...
			if ( (indexZeile >= (k+1)*MPCDenisSimulation.stepLength(TimeUnit.MINUTES)) && (buffer.length != 0) ) {
				try {
					for(int j = 0; j < buffer.length; j++) {
						
						//*********das ist nötig, falls der Zeitschritt nicht genau einer minute entspricht.
						
						double deltaOverMinute = indexZeile - (k+1) * MPCDenisSimulation.stepLength(TimeUnit.MINUTES);
	    ...
	    int daysToConsider = (int) Math.round(MPCDenisSimulation.N_STEPS/k + 0.5);
