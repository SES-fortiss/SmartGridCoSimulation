...
public class Raspberry extends Thread implements ChangeListener {
	...
	public int istCluster = 8000; // Startwert
	...
	private int istSolarFortiss = 1200; // aktueller Wert
	private int istFutureSolarFortiss = 1200; // Bald erreichter Wert

	private int istWindrad; // aktueller Wert
	private int istFutureWindrad; // Bald erreichter Wert

	private int istSolar = 800; // aktueller Wert
	private int istFutureSolar = 800; // Bald erreichter Wert
	...
	public void run() {
		...
		while (run) {
			...
			setistFutureBiogas(istCluster - istFutureWindrad - istFutureSolar
			...
			istRequest = istCluster - istIst;

			PiHandler.requestPower = istRequest;
			PiHandler.setPoint = istCluster;
	...
	private boolean helpupdateistRandomValues() {
		...
		if (istBiogas != istFutureBiogas) {
			...
			int neu =
					this.istCluster - this.istSolar - this.istSolarFortiss
							- this.istWindrad;
			if (neu - act > 0) {
